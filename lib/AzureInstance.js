...
export default class AzureInstance {
  ...
  getOpeningHours(location) {
    let locationParam = 1; //default BZ
    switch (location) {
      case "BZ":
        locationParam = 1;
        break
    ...
    return fetch("https://webservices.scientificnet.org/rest/cm/mensa/api/openinghours/?locations=" + locationParam + "&language=en", {
      headers: {
        Authorization: "Bearer " + this.getToken().accessToken
      }
    })
